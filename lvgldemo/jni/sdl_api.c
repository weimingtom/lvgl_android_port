...
#else
	width = LV_HOR_RES_MAX;// 480;// 320; //LV_HOR_RES
	height = LV_VER_RES_MAX;// 320;// 240; //LV_VER_RES
	MainFrameResize(width, height);
	//Nofrendo_main_entry();
	main();
#endif
...
//see LV_HOR_RES_MAX
extern int32_t my_screen[480 * 320];// = {0};
void CanvasDrawPixel(int x1, int y1, int color, int alpha)
{
	if (x1 >= 0 && x1 < 480 && y1 >= 0 && y1 < 320) {
		//my_screen[y1 * 480 + x1] = ((0xFF000000) | (0xFF0000 & 0xffffff));

		//my_screen[y1 * 480 + x1] = (((0xff) << 24) | (color & 0xffffff));

		unsigned char R = (color & 0xFF) >> 0;
		unsigned char G = (color & 0xFF00) >> 8;
		unsigned char B = (color & 0xFF0000) >> 16;

		my_screen[y1 * 480 + x1] = (((alpha & 0xff) << 24) | (( (R << 16) | (G << 8) | B) & 0xffffff));
	}
}
...
{
#if 1
	for (int y1_ = 0; y1_ < 320; ++y1) {
		for (int x1_ = 0; x1_ < 480; ++x1) {
			//my_screen[y1 * 480 + x1] = ((0xFF000000) | (0x00FF00 & 0xffffff));
			...
			if (y1_ >= y1 && y1_ < y2) {
				if (x1_ >= x1 && x1_ < x2) {
					...
					my_screen[y1_ * 480 + x1_] = (((0xff) << 24) | (( (R << 16) | (G << 8) | B) & 0xffffff));
				}
			}
		}
	}
